...
namespace IssuesLog.Controllers
{
    public class IssuesController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var CurrentUserName = strUserDetails.Split('\\')[1];
            CurrentUserName = char.ToUpper(CurrentUserName[0]) + CurrentUserName.Substring(1).ToLower();
            ViewBag.UserName = CurrentUserName;

            // Filter issues submitted by the current user
            var issues = db.Issuess
                .Where(i => i.SubmittedBy == CurrentUserName)
        ...
        public ActionResult Create(Issue issue, IEnumerable<HttpPostedFileBase> attachments)
        {
            if (ModelState.IsValid)
            {
                ...
                issue.SubmittedBy = ViewBag.UserName;
                ...
                if (cxRoleId != Guid.Empty)
                {
                    ...
                    // Safely format base URL
                    string baseUrl = ConfigurationManager.AppSettings["SystemBaseUrl"];
                    string issuePath = $"/CX/Assign/{issue.IssueId}";
                    string loginUrl = $"{baseUrl.TrimEnd('/')}/Home/Login?ReturnUrl={HttpUtility.UrlEncode(issuePath)}";
                    ...
                    string emailBody = $@"
                        ...
                        <table border='1' cellpadding='8' cellspacing='0' style='border-collapse: collapse; font-family: Arial, sans-serif; font-size: 14px;'>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>View Issue</td>
                                    <td><a href='{loginUrl}' target='_blank'>🔗 View Issue (Login Required)</a></td>
                                </tr>
                            </tbody>
                        </table>
                        <br/>
                        <p>Kind Regards,<br/><strong>{ViewBag.UserName}</strong></p>";
